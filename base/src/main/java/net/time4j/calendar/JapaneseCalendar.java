...
public final class JapaneseCalendar
    ...
    public static final StdCalendarElement<Integer, JapaneseCalendar> DAY_OF_MONTH =
        new StdIntegerDateElement<>("DAY_OF_MONTH", JapaneseCalendar.class, 1, 31, 'd');
    ...
    public static final StdCalendarElement<Integer, JapaneseCalendar> DAY_OF_YEAR =
        new StdIntegerDateElement<>("DAY_OF_YEAR", JapaneseCalendar.class, 1, 365, 'D');
    ...
    private static final WeekdayInMonthElement<JapaneseCalendar> WIM_ELEMENT =
        new WeekdayInMonthElement<>(JapaneseCalendar.class, DAY_OF_MONTH, DAY_OF_WEEK);
    ...
    public static JapaneseCalendar of(
        ...
        Optional<Nengo> next = nengo.findNext();

        if (next.isPresent() && (next.get().getFirstRelatedGregorianYear() < relgregyear)) {
    ...
    private static Nengo findBestNengo(
        ...
        while (nengo.getStartAsDaysSinceEpochUTC() > utcDays) {
            Optional<Nengo> previous = nengo.findPrevious();
    ...
    private static class JapaneseUnitRule
        ...
        private static JapaneseCalendar erasAdded(
            ...
            Nengo nengoNew = Nengo.ofIndexOfficial(Math.addExact(nengo.getIndexOfficial(), MathUtils.safeCast(amount)));
            Optional<Nengo> next = nengoNew.findNext();

            if (next.isPresent()) {
                int ymax = next.get().getFirstRelatedGregorianYear() - nengoNew.getFirstRelatedGregorianYear() + 1;
                if (yon > ymax) {
                    yon = ymax;
                }
            }
    ...
    private static class NengoRule
        ...
        public JapaneseCalendar withValue(
            ...
            Optional<Nengo> next = value.findNext();

            if (next.isPresent()) {
                int ymax = next.get().getFirstRelatedGregorianYear() - value.getFirstRelatedGregorianYear() + 1;
                if (yon > ymax) {
                    yon = ymax;
                }
            }
        ...
        int getMax(JapaneseCalendar context) {
            ...
            switch (this.index) {
                case YEAR_OF_NENGO_INDEX:
                    ...
                    Optional<Nengo> next = current.findNext();
                    if (next.isPresent()) {
                        // we allow for example Shōwa 64 (equivalent to Heisei 1)
                        return next.get().getFirstRelatedGregorianYear() - current.getFirstRelatedGregorianYear() + 1;
                    } else {
