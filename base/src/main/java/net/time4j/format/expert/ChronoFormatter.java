...
public final class ChronoFormatter<T>
     ...
     * </ul>
     *
     * <p>Note that this check will only be done here but not on builder level (since v4.20). The
     ...
     * durchgef&uuml;hrt (seit v4.20). Sie hat auch nicht den Anspruch, alle ungesunden Kombinationen
    ...
    private static ChronoFormatter<Moment> rfc1123() {
        ...
        rfc1123DateTime(builder);
        final Map<String, ZonalOffset> northAmericanZones = new HashMap<>();
                ...
                while (j < n) {
                    if (pattern.charAt(j) == '\'') {
                        if ((j + 1 < n) && (pattern.charAt(j + 1) == '\'')) {
                            ...
                            if (z && (j == i + 2) && hasUnixChronology(builder.chronology)) {
    ...
    private List<FormatStep> freeze(List<FormatStep> steps) {
        ...
        List<FormatStep> frozen = new ArrayList<>(steps.size());
