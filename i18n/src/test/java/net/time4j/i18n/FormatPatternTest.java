...
public class FormatPatternTest {
    ...
    public void timePattern() {
        ...
        assertThat(
            ChronoFormatter.ofTimeStyle(DisplayMode.FULL, Locale.GERMANY).format(time),
            is("17:45 Uhr")); // ohne Offset!!!
        assertThat(
            ChronoFormatter.ofTimeStyle(DisplayMode.LONG, Locale.GERMANY).format(time),
            is("17:45:30")); // ohne Offset!!!
        assertThat(
            ChronoFormatter.ofTimeStyle(DisplayMode.MEDIUM, Locale.GERMANY).format(time),
            is("17:45:30")); // ohne Offset!!!
        assertThat(
            ChronoFormatter.ofTimeStyle(DisplayMode.SHORT, Locale.GERMANY).format(time),
    ...
    public void dateTimePattern() {
        ...
        assertThat(
            ChronoFormatter.ofMomentStyle(DisplayMode.FULL, DisplayMode.FULL, Locale.GERMANY, Timezone.of("Europe/Berlin").getID()).format(m),
            is("Donnerstag, 1. Januar 1970 um 01:00:00 Mitteleuropäische Zeit"));
        assertThat(
            ChronoFormatter.ofMomentStyle(DisplayMode.LONG, DisplayMode.LONG, Locale.GERMANY, Timezone.of("Europe/Berlin").getID()).format(m),
            is("1. Januar 1970 um 01:00:00 MEZ"));
        assertThat(
            ChronoFormatter.ofMomentStyle(DisplayMode.MEDIUM, DisplayMode.MEDIUM, Locale.GERMANY, Timezone.of("Europe/Berlin").getID()).format(m),
            is("01.01.1970, 01:00:00"));
        assertThat(
            ChronoFormatter.ofMomentStyle(DisplayMode.SHORT, DisplayMode.SHORT, Locale.GERMANY, Timezone.of("Europe/Berlin").getID()).format(m),
            is("01.01.70, 01:00"));
        assertThat(
