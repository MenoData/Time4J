...
public class TwilightTest {
    ...
    public void hamburgWilliams() {
        ...
        assertThat(
            date.get(hamburg.sunrise(Twilight.NAUTICAL)).toZonalTimestamp(tzid).toTime(),
            is(PlainTime.of(5, 15)));
        assertThat(
            date.get(hamburg.sunrise(Twilight.CIVIL)).toZonalTimestamp(tzid).toTime(),
            is(PlainTime.of(6, 0)));
        assertThat(
            date.get(hamburg.sunset(Twilight.ASTRONOMICAL)).toZonalTimestamp(tzid).toTime(),
            is(PlainTime.of(22, 7)));
        assertThat(
            date.get(hamburg.sunset(Twilight.NAUTICAL)).toZonalTimestamp(tzid).toTime(),
            is(PlainTime.of(21, 18)));
        assertThat(
            date.get(hamburg.sunset(Twilight.CIVIL)).toZonalTimestamp(tzid).toTime(),
